...
状态：v3.3.0 已正式发布；v3.3.1 候选实现、自动化验收与真实 Chrome 桌面交互验收已完成。
Firefox 与真实触摸手势的发布回归仍按 `v3.3.1-plan.md` 执行。
...
- 自动播放开关只在 Reader 会话内有效；间隔默认 5 秒并持久化为毫秒，当前设置面板允许直接
  输入 1–60 秒整数或用加减按钮按 5 秒调整；输入、持久化和 timer 均以 1 秒为硬下限。运行中的
  Reader 收到设置变更后会重启唯一计时器。
...
- 图片来源填充和结构重映射均不得在 Driver 手势、主滚动或动画期间刷新 Slide；交互结束后
  ...
  调度前直接调用 Driver 刷新。布局请求由单一合并调度器处理：正常过渡使用短 RAF burst，
  长时间交互改为有界低频重试，任何异常状态都不得形成永久逐帧轮询。Driver 只把会移动 holder
  的手势、主滚动、开合和 pan 动画视为阻塞，不相关的 PhotoSwipe/宿主动画不能永久阻塞刷新。
  过渡期间保持 HUD，pending 槽不得是无反馈的纯黑区域。相同 HUD 状态不得重复重建 DOM，状态
  气泡不接收指针事件。桌面端
  可在当前 Spread 未完整挂载时保留控件作兜底；触摸设备的状态 HUD 与控件可见性必须解耦，
  加载状态不得唤醒或延长页码、关闭按钮和缩略图面板的显示，只能由用户主动 tap 唤醒控件。
...
- Reader 的“当前内容已加载”不仅要求图片生命周期完成，还要求索引一致的当前 Slide 容器仍
  ...
  并在正常 `change` 结束后的空闲帧防御性修复当前 holder；触摸端控件仍遵守独立自动隐藏规则。
- 图片节点仅有 `src` 不代表展示层已经就绪；当前 Spread 的每个成员还必须完成解码且具有有效
  `naturalWidth/naturalHeight`。Driver 在展示节点的 `load/error` 边界发布通用状态，当前节点失败时
  只允许一次有界原位重试；重试仍失败则保持错误 HUD，不得把共享字节任务的 `loaded` 状态误当作
  可见像素，也不得形成无限重试或站点名称分支。
