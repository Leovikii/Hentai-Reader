...
本文件只用于追溯设计原因。当前实现边界见 `architecture.md`；存在活动版本计划时，当前目标以其为准。
...
结果：已正式发布。115/115 项测试、类型检查、生产构建、差异检查与最终实机验收通过；脚本为
374,152 字节，gzip 79.57 kB。

- 自动播放默认间隔改为 5 秒；按钮按 5 秒调整，输入框接受 1–60 秒整数，所有入口以 1 秒为
  硬下限。设置面板隔离 Reader 的焦点与键盘事件，阅读器开启时仍可直接输入，同时保留
  PhotoSwipe 原有焦点陷阱和导航配置。
- 自动播放改为单个 one-shot timer：正常间隔结束时若当前 Spread 尚未完整显示，只追加一次
  固定 5 秒宽限，随后继续翻页，不再由图片或布局事件反复暂停、恢复。
- E-Hentai viewer HTML 请求增加前台 12 秒、后台 20 秒单次超时，避免挂起请求永久占用站点
  limiter；超时只终止该站点请求，不改变共享图片生命周期所有权。
- 动态双页新增持久化的 LTR/RTL 排列选项，仅在双页功能开启时显示。方向只交换同一 Spread
  内的左右展示位置，不改变逻辑配对、索引、导航、手势、分页、预取或 PhotoSwipe 根实例。
- 设置与悬浮 UI 禁止文本拖选，自动播放数字输入框仍保留选择和编辑能力；桌面窄视口的设置
  浮层保持 16 px 安全边距。
