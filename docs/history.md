...
结果：已完成自动化、本地 Chrome fixture 与 E-Hentai 真实图库验收。
...
- 18comic 受管资源使用 24 个 Object URL / 96 MiB Blob 双预算，并只对所有权型离屏图片
  恢复占位、释放租约。
- 设置与控制改为语义化按钮，补齐 inert、aria、焦点恢复、控制条恢复菜单和自动播放下限。
- PNG 图标替换为纯 SVG；userscript 图标 data URL 为 1,493 字符，较旧 PNG 元数据减少
  约 15 KiB。
