...
- 无廉价原始缩略图时维持当前占位/已加载图派生策略；完整派生缩略图方案不阻塞架构封板，
  但已升级为 v3.2.0 发布前必须完成的最后一个功能。
...
- 18comic 派生缩略图曾是 v3.2.0 发布前必须完成的最后一个功能，现已实现并验收。不得恢复
  大范围原图缩略图加载；最终方案、性能边界和验收结果见 `docs/refactor-backlog.md`。
- 采用通用稳定可视批次：只有用户主动滚动后稳定 300ms 才触发；实际可视范围全部排队、
  中心优先、并发 2，不包含虚拟列表 Buffer。URL/Sprite 原生缩略图路径保持不变。
- 无原生预览的条目直接使用共享图片加载租约，不再通过卷轴 placeholder 间接加载；
  再次滚动、Reader 导航或关闭时释放缩略图专用任务；面板自动隐藏不打断已确认的有限批次，
  Reader 点击复用并提升同一任务。
- 真实 18comic 图库已验证稳定后渐进填充、共享加载和反乱序，桌面与移动端功能正常。
- E-Hentai URL/Sprite 与 4KHD `w300-h300-rw` 原生缩略图路径回归通过；两者不进入
  `none/derived` 原图派生队列，行为符合按预览能力分层的设计。
- 通用实现、42 项自动化、本地浏览器 smoke、三站真实站点验收和最终 `npm run check`
  均已通过；v3.2.0 最后一个功能发布阻塞项已关闭，可以进入 PR、合并和 Release 流程。
...
1. 阶段 1–11 已完成并封板；架构重构不再扩展。
2. 18comic 派生缩略图已实现并完成三站真实回归，v3.2.0 功能开发结束。
3. 最终完整检查已通过；审阅差异后完成 v3.2.0 PR、合并和 Release。
4. 定位机制维持一次逻辑页跳转，不重新引入关闭后定时器、观察器或二次滚动修正。
5. 维持现有数量 LRU，不增加字节预算、保留定时器、DOM 窗口化或站点特例。
6. 新站点按 `_template` 和 `new-site-guide.md` 接入，不复制 Reader、加载、重试和缩略图逻辑。
